...
import {
  ...
  Edit3, Calendar, Sparkles, MapPin, DollarSign
} from "lucide-react";
...
export default async function PitchDeckView({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch pitch deck along with full author profile data from the database
  ...
  // Sidebar Live Database Quick Stats Mapping
  const displayTier = pitchDeck.author?.tier || "Freemium";
  const displayLocation = [pitchDeck.author?.city, pitchDeck.author?.country].filter(Boolean).join(", ") || pitchDeck.author?.country || "Global Network";
  const targetRaise = fundingGoal;
  const primarySector = pitchDeck.industry || pitchDeck.author?.industry || "Technology";
  ...
  return (
    <div className="min-h-screen bg-[var(--primary)] text-[var(--secondary)] flex flex-col justify-between relative transition-colors duration-300">
      ...
      <main className="pt-32 pb-24 px-4 md:px-6 mx-auto max-w-7xl w-full relative z-10 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* RIGHT SIDEBAR: Live Database Quick Stats (3 Columns) */}
          <div className="lg:col-span-3 flex flex-col gap-4">
            <div className="neu-flat-base p-5 flex flex-col gap-2">
              <span className="text-[10px] text-[var(--secondary)]/60 font-bold uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles size={14} className="text-[var(--accent)]" /> Membership Tier
              </span>
              <span className="text-xl font-black text-[var(--secondary)] capitalize pl-1">{displayTier}</span>
            </div>

            <div className="neu-flat-base p-5 flex flex-col gap-2">
              <span className="text-[10px] text-[var(--secondary)]/60 font-bold uppercase tracking-wider flex items-center gap-1.5">
                <MapPin size={14} className="text-[var(--accent)]" /> Headquarters
              </span>
              <span className="text-xl font-black text-[var(--secondary)] truncate pl-1">{displayLocation}</span>
            </div>

            <div className="neu-flat-base p-5 flex flex-col gap-2">
              <span className="text-[10px] text-[var(--secondary)]/60 font-bold uppercase tracking-wider flex items-center gap-1.5">
                <DollarSign size={14} className="text-[var(--accent)]" /> Target Raise
              </span>
              <span className="text-xl font-black text-[var(--secondary)] pl-1">{targetRaise}</span>
            </div>

            <div className="neu-flat-base p-5 flex flex-col gap-2">
              <span className="text-[10px] text-[var(--secondary)]/60 font-bold uppercase tracking-wider flex items-center gap-1.5">
                <Briefcase size={14} className="text-[var(--accent)]" /> Primary Sector
              </span>
              <span className="text-xl font-black text-[var(--secondary)] truncate pl-1">{primarySector}</span>
            </div>
          </div>
